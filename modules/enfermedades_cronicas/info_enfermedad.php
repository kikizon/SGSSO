...
if (!$enfermedad_id) {
    echo '<div class="alert alert-danger">ID de enfermedad no válido.</div>';
    exit;
}
...
if (!$enfermedad) {
    echo '<div class="alert alert-warning">Enfermedad no encontrada.</div>';
    exit;
}

// Empleados que SÍ tienen la enfermedad
$paramsTienen = array_merge([$enfermedad_id], $paramsBase);
$sqlTienen = "SELECT e.id, e.numero_empleado, e.nombre, d.nombre as departamento, s.nombre as sucursal, ee.fecha_registro
...
// Empleados que NO tienen la enfermedad
...
<div class="modal-header bg-info text-white">
    <h5 class="modal-title"><i class="fas fa-chart-pie"></i> Cobertura de Enfermedad: <?= htmlspecialchars($enfermedad['nombre']) ?></h5>
...
<div class="modal-body">
    ...
    <div class="card mb-3">
        <div class="card-header p-2">
            ...
            <a href="exportar_cobertura.php?id=<?= $enfermedad_id ?>&<?= http_build_query($_GET) ?>" class="btn btn-sm btn-success ms-2 no-spinner"><i class="fas fa-file-excel"></i> Exportar</a>
        </div>
        <div class="collapse" id="filtrosCobertura">
            <div class="card-body">
                <form id="formFiltrosCobertura" data-enfermedad-id="<?= $enfermedad_id ?>" class="row g-2">
                    <div class="col-md-3">
    ...
    <div class="tab-content mt-3">
        <div class="tab-pane fade show active" id="tienen" role="tabpanel">
            <?php if (empty($tienen)): ?>
                <div class="alert alert-info">Ningún empleado tiene esta enfermedad.</div>
            <?php else: ?>
                    ...
                    <table class="table table-sm table-hover">
                        ...
                        <tbody>
                            ...
                            <tr id="emp-<?= $e['id'] ?>">
                                ...
                                <td>
                                    <button class="btn btn-sm btn-outline-danger btn-desmarcar" data-empleado-id="<?= $e['id'] ?>" data-enfermedad-id="<?= $enfermedad_id ?>" title="Marcar como NO tiene">
        ...
        <div class="tab-pane fade" id="noTienen" role="tabpanel">
            <?php if (empty($noTienen)): ?>
                <div class="alert alert-success">¡Todos los empleados tienen esta enfermedad!</div>
            <?php else: ?>
                <div class="table-responsive">
                    <table class="table table-sm table-hover">
                        <thead class="table-light">
                        ...
                        <tbody>
                            ...
                            <tr id="emp-<?= $e['id'] ?>">
                                ...
                                <td><?= htmlspecialchars($e['sucursal']) ?></td>
                                <td>
                                    <button class="btn btn-sm btn-outline-success btn-marcar" data-empleado-id="<?= $e['id'] ?>" data-enfermedad-id="<?= $enfermedad_id ?>" title="Marcar como tiene">
