...
$sql = "SELECT e.*, 
               (SELECT COUNT(DISTINCT ee.empleado_id) 
                FROM empleado_enfermedad ee 
                JOIN empleados emp ON ee.empleado_id = emp.id 
                WHERE ee.enfermedad_id = e.id AND emp.activo = 1) as total_empleados
        FROM enfermedades_cronicas e
        $whereEstado
        ORDER BY e.nombre";
...
<h2><i class="fas fa-heartbeat"></i> Catálogo de Enfermedades Crónicas</h2>
<a href="crear.php" class="btn btn-primary mb-3"><i class="fas fa-plus"></i> Nueva Enfermedad</a>
    ...
    <table class="table table-striped">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><?= $e['total_empleados'] > 0 ? '<span class="badge bg-info">'.$e['total_empleados'].'</span>' : '<span class="text-muted">—</span>' ?></td>
                <td><?= $e['activo'] ? '<span class="badge bg-success">Activo</span>' : '<span class="badge bg-danger">Inactivo</span>' ?></td>
                <td>
                    <button class="btn btn-sm btn-outline-info ver-cobertura" data-id="<?= $e['id'] ?>" data-nombre="<?= htmlspecialchars($e['nombre']) ?>" title="Ver cobertura"><i class="fas fa-chart-pie"></i></button>
                    <a href="editar.php?id=<?= $e['id'] ?>" class="btn btn-sm btn-warning" title="Editar"><i class="fas fa-edit"></i></a>
                    <a href="eliminar.php?id=<?= $e['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('¿Eliminar?')" title="Eliminar"><i class="fas fa-trash"></i></a>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
...
function recargarCobertura(enfermedadId, filtros = {}) {
    ...
    const params = new URLSearchParams({id: enfermedadId, ...filtros});
    fetch(`info_enfermedad.php?${params.toString()}`)
        ...
        .then(html => {
            ...
            setTimeout(() => {
                ...
                if (pestañaActiva === 'tienen') {
                    tabTienen?.classList.add('active'); tabNoTienen?.classList.remove('active');
                    paneTienen?.classList.add('show', 'active'); paneNoTienen?.classList.remove('show', 'active');
                } else {
                    tabNoTienen?.classList.add('active'); tabTienen?.classList.remove('active');
                    paneNoTienen?.classList.add('show', 'active'); paneTienen?.classList.remove('show', 'active');
                }
            }, 50);
        })
...
function aplicarFiltrosCobertura() {
    ...
    if (!form) return;
    const enfermedadId = form.dataset.enfermedadId;
    ...
    recargarCobertura(enfermedadId, filtros);
}

function limpiarFiltrosCobertura() {
    const form = document.getElementById('formFiltrosCobertura');
    if (!form) return;
    form.sucursal_id.value = ''; form.departamento_id.value = ''; form.estado.value = '1'; form.buscar.value = '';
    aplicarFiltrosCobertura();
}

async function toggleEnfermedad(empleadoId, enfermedadId, accion, rowElement) {
    const formData = new FormData();
    formData.append('empleado_id', empleadoId);
    formData.append('enfermedad_id', enfermedadId);
    formData.append('accion', accion);
    try {
        const response = await fetch('marcar_enfermedad.php', { method: 'POST', body: formData });
        ...
        if (data.success) {
            ...
            rowElement.style.transition = 'opacity 0.3s'; rowElement.style.opacity = '0';
            setTimeout(() => {
                rowElement.remove();
                const nuevaFila = document.createElement('tr');
                nuevaFila.id = `emp-${empleadoId}`;
                nuevaFila.style.opacity = '0'; nuevaFila.style.transition = 'opacity 0.3s';
                if (accion === 'tiene') {
                    nuevaFila.innerHTML = `<td>${numero}</td><td>${nombre}</td><td>${depto}</td><td>${suc}</td><td>${new Date().toLocaleDateString('es-MX')}</td>
                        <td><button class="btn btn-sm btn-outline-danger btn-desmarcar" data-empleado-id="${empleadoId}" data-enfermedad-id="${enfermedadId}"><i class="fas fa-times"></i> No tiene</button></td>`;
                    ...
                    nuevaFila.innerHTML = `<td>${numero}</td><td>${nombre}</td><td>${depto}</td><td>${suc}</td>
                        <td><button class="btn btn-sm btn-outline-success btn-marcar" data-empleado-id="${empleadoId}" data-enfermedad-id="${enfermedadId}"><i class="fas fa-check"></i> Tiene</button></td>`;
                    document.querySelector('#noTienen tbody').appendChild(nuevaFila);
                }
                setTimeout(() => { nuevaFila.style.opacity = '1'; }, 10);
                actualizarContadores();
            }, 300);
        } else alert('Error: ' + (data.error || 'No se pudo realizar'));
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.ver-cobertura').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            filtrosActuales = {}; pestañaActiva = 'tienen';
            modalContent.innerHTML = '<div class="text-center p-3"><div class="spinner-border"></div> Cargando...</div>';
            modal.show();
    ...
    document.body.addEventListener('click', function(e) {
        if (e.target.closest('#btnFiltrarCobertura')) { e.preventDefault(); aplicarFiltrosCobertura(); }
        if (e.target.closest('#btnLimpiarCobertura')) { e.preventDefault(); limpiarFiltrosCobertura(); }
        const btnMarcar = e.target.closest('.btn-marcar');
        if (btnMarcar) {
            e.preventDefault();
            toggleEnfermedad(btnMarcar.dataset.empleadoId, btnMarcar.dataset.enfermedadId, 'tiene', btnMarcar.closest('tr'));
        }
        const btnDesmarcar = e.target.closest('.btn-desmarcar');
        if (btnDesmarcar) {
            e.preventDefault();
            toggleEnfermedad(btnDesmarcar.dataset.empleadoId, btnDesmarcar.dataset.enfermedadId, 'no_tiene', btnDesmarcar.closest('tr'));
        }
    });
});
</script>
