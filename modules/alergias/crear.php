...
if (!verify_csrf_token($_POST['csrf_token'] ?? '')) { $error = 'Token de seguridad inválido. Recarga la página e intenta de nuevo.'; }
    ...
    if ($nombre === '') {
        ...
        $stmt = $pdo->prepare("INSERT INTO alergias (nombre, descripcion) VALUES (?, ?)");
        try {
            $stmt->execute([$nombre, $descripcion]);
            $success = 'Alergia creada exitosamente.';
            $_POST = [];
...
<h2><i class="fas fa-plus-circle"></i> Nueva Alergia</h2>
