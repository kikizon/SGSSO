...
exigir('empleados.historial.exportar');
...
$stmt = $pdo->prepare("SELECT nombre FROM alergias WHERE id = ?");
...
$sqlTienen = "SELECT e.numero_empleado, e.nombre, d.nombre as departamento, s.nombre as sucursal, ea.fecha_registro
...
$sheet1->setTitle('Tienen la alergia');
$sheet1->setCellValue('A1', 'Alergia: ' . $alergia['nombre']);
$sheet1->setCellValue('A2', 'Empleados que TIENEN la alergia (filtros aplicados)');
...
$sheet2->setTitle('NO tienen la alergia');
$sheet2->setCellValue('A1', 'Alergia: ' . $alergia['nombre']);
$sheet2->setCellValue('A2', 'Empleados que NO tienen la alergia (filtros aplicados)');
...
header('Content-Disposition: attachment;filename="cobertura_' . preg_replace('/[^a-zA-Z0-9]/', '_', $alergia['nombre']) . '_' . date('Ymd_His') . '.xlsx"');
