...
if ($es_admin) {
    ...
    $st->execute([$usuario_sucursal_id]);
...
$f_desde    = $_GET['fecha_desde'] ?? date('Y-m-d', strtotime('monday this week'));
...
if ($datos) {
    ...
    $st = $pdo->prepare("SELECT COUNT(*) AS total, ROUND(AVG(evaluacion_total),1) AS prom
                         FROM auditorias_6s
                         WHERE estado = 'finalizada' AND sucursal_id = ? AND fecha BETWEEN ? AND ?");
    $st->execute($base);
    $kpi = $st->fetch();

    // Promedio por departamento en el rango
    $st = $pdo->prepare("SELECT d.id, d.nombre, ROUND(AVG(a.evaluacion_total),1) AS prom, COUNT(*) AS n
    ...
    $st->execute($base);
    $dep_scores = $st->fetchAll();

    // Promedio por categoría 6S por departamento (para radares; no contestados = 0)
    $st = $pdo->prepare("SELECT a.departamento_id AS dep_id, cat.nombre AS categoria, cat.orden AS cat_orden,
                         ...
                         WHERE a.estado = 'finalizada' AND a.sucursal_id = ? AND a.fecha BETWEEN ? AND ?
                         GROUP BY a.departamento_id, cat.id
                         ORDER BY cat.orden");
    $st->execute($base);
    $cat_dep = []; // dep_id => ['labels'=>[], 'data'=>[]]
    foreach ($st->fetchAll() as $row) {
        $cat_dep[$row['dep_id']]['labels'][] = $row['categoria'];
        $cat_dep[$row['dep_id']]['data'][]   = (float)$row['prom'];
    }

    // Detalle de respuestas calificadas; se agrega en PHP para incluir observaciones, fechas y auditores
    $st = $pdo->prepare("SELECT d.id AS dep_id, cat.nombre AS categoria, cat.orden AS cat_orden,
                                cr.id AS crit_id, cr.texto,
    ...
    foreach ($st->fetchAll() as $row) {
        ...
        if ((int)$row['puntaje'] < 100) {
            $agg[$dep][$cid]['fallas']++;
            $agg[$dep][$cid]['detalles'][] = [
                'fecha' => $row['fecha'], 'auditor' => $row['auditor'], 'puntaje' => (int)$row['puntaje'],
                'calificacion' => $row['calificacion'], 'comentarios' => $row['comentarios'],
                'prioridad' => $row['prioridad'], 'fecha_compromiso' => $row['fecha_compromiso'],
            ];
        }
    }
    // Solo criterios con al menos una falla, ordenados por recurrencia
    foreach ($agg as $dep => $crits) {
        $lista = [];
        foreach ($crits as $c) {
            if ($c['fallas'] > 0) {
                $c['prom'] = $c['evaluado'] ? round($c['suma'] / $c['evaluado'], 1) : 0;
                $lista[] = $c;
            }
        }
...
<style>
  * { -webkit-print-color-adjust: exact; print-color-adjust: exact; }
  ...
  @media print {
    ...
    .radar-box { max-width: 300px !important; margin: 0 auto; }
    .radar-box canvas { max-width: 100% !important; height: auto !important; }
    .print-head {
      display: flex; align-items: center; gap: 16px;
      border-bottom: 2px solid #0d6efd; padding-bottom: 10px; margin-bottom: 12px;
    }
    .print-head img { height: 56px; }
    .print-head h1 { font-size: 18px; margin: 0; }
    .print-head .sub { color: #555; font-size: 11px; }
  }
</style>
...
<div class="print-head">
  ...
  <div>
    <h1>Hoja de Resumen 6S</h1>
    <div class="sub">SUPERMM SYSO · <?= htmlspecialchars($suc_nombre ?: '') ?> · <?= format_date_es($f_desde) ?> — <?= format_date_es($f_hasta) ?></div>
  </div>
</div>
...
<p class="text-muted small screen-only">Solo auditorías <strong>finalizadas</strong>. Se listan los puntos donde el departamento <strong>no alcanzó 100%</strong>, ordenados por recurrencia, para detectar las fallas más frecuentes.</p>
...
<div class="card mb-3">
  <div class="card-body">
    <div class="row align-items-center">
      <div class="col-md-4">
        ...
        <div class="row mt-3">
          <div class="col-6">
            ...
            <div class="h2 mb-0"><span class="badge bg-<?= color_6s($kpi['prom']) ?>"><?= $kpi['prom'] !== null ? $kpi['prom'].'%' : '—' ?></span></div>
          </div>
          <div class="col-6">
      ...
      <div class="col-md-8">
        <?php if (!empty($dep_scores)): ?>
          <div class="radar-box mx-auto" style="max-width:430px;"><canvas id="radarSucursal"></canvas></div>
        <?php endif; ?>
      </div>
    </div>
  </div>
...
<?php if (!$datos || (int)$kpi['total'] === 0): ?>
  <div class="alert alert-info">No hay auditorías finalizadas para esa sucursal y rango de fechas.</div>
<?php else: ?>
  <?php foreach ($dep_scores as $d):
        $items = $fallas[$d['id']] ?? []; ?>
  <div class="card mb-3">
    <div class="card-header d-flex justify-content-between align-items-center">
      <strong><?= htmlspecialchars($d['nombre']) ?></strong>
      <span>
        <span class="text-muted small me-2"><?= (int)$d['n'] ?> auditoría(s)</span>
        <span class="badge bg-<?= color_6s($d['prom']) ?>"><?= $d['prom'] ?>%</span>
      </span>
    </div>
    <div class="card-body p-0">
      <?php if (!empty($cat_dep[$d['id']])): ?>
      <div class="p-2 border-bottom d-flex justify-content-center">
        <div class="radar-box" style="max-width:340px;width:100%;">
          <canvas class="radarDepto" data-dep="<?= (int)$d['id'] ?>"></canvas>
        </div>
      </div>
      <?php endif; ?>
      <?php if (empty($items)): ?>
      ...
      <?php else: ?>
        <table class="table table-sm mb-0">
          <thead>
            <tr>
              <th>Categoría</th>
              <th>Punto a mejorar</th>
              <th class="text-center">Veces sin 100%</th>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>
                <div class="fw-semibold"><?= htmlspecialchars($it['texto']) ?></div>
                <div class="small text-muted mt-1">
                  <?php foreach ($it['detalles'] as $de): ?>
                    <div class="mb-1">
                      <i class="fas fa-circle" style="font-size:.4rem;vertical-align:middle;"></i>
                      <?= format_date_es($de['fecha']) ?> ·
                      <i class="fas fa-user-check"></i> <?= htmlspecialchars($de['auditor'] ?? 's/auditor') ?> ·
...
<script>
const RADAR_DEP = {
  labels: <?= json_encode(array_map(fn($d) => $d['nombre'], $dep_scores)) ?>,
  data:   <?= json_encode(array_map(fn($d) => $d['prom'] !== null ? (float)$d['prom'] : 0, $dep_scores)) ?>
};
...
const radarOpts = {
  ...
  animation: false, // sin animación: queda listo de inmediato para imprimir
  devicePixelRatio: 2,
  scales: { r: { min: 60, max: 100, ticks: { stepSize: 10, font: { size: 9 } }, pointLabels: { font: { size: 10 } } } },
  plugins: { legend: { display: false } }
};

if (document.getElementById('radarSucursal')) {
  new Chart(document.getElementById('radarSucursal'), {
    type: 'radar',
    data: { labels: RADAR_DEP.labels,
      datasets: [{ data: RADAR_DEP.data, backgroundColor: 'rgba(13,110,253,.2)',
...
document.querySelectorAll('.radarDepto').forEach(cv => {
  const d = RADAR_CAT[cv.dataset.dep];
  if (!d) return;
  new Chart(cv, {
    type: 'radar',
    data: { labels: d.labels,
      datasets: [{ data: d.data, backgroundColor: 'rgba(25,135,84,.2)',
        borderColor: '#198754', pointBackgroundColor: '#198754' }] },
    options: radarOpts
  });
});
</script>
