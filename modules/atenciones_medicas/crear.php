...
exigir('atenciones_medicas.crear');
...
if (!verify_csrf_token($_POST['csrf_token'] ?? '')) { $error = 'Token de seguridad inválido. Recarga la página e intenta de nuevo.'; }
    ...
    if ($descripcion === '') {
        ...
        $stmt = $pdo->prepare("INSERT INTO atenciones_medicas (descripcion) VALUES (?)");
        try {
            $stmt->execute([$descripcion]);
            $success = 'Atención médica creada.';
            $_POST = [];
...
<h2><i class="fas fa-plus-circle"></i> Nueva Atención Médica</h2>
