...
if (!verify_csrf_token($_POST['csrf_token'] ?? '')) { $error = 'Token de seguridad inválido. Recarga la página e intenta de nuevo.'; }
    ...
    if (!$descripcion) {
        ...
        $stmt = $pdo->prepare("INSERT INTO actos_inseguros (descripcion, activo) VALUES (?, ?)");
        try {
            $stmt->execute([$descripcion, $activo]);
            $success = 'Acto inseguro creado exitosamente.';
            $_POST = [];
...
<h2><i class="fas fa-plus-circle"></i> Nuevo Acto Inseguro</h2>
...
<form method="post" class="row g-3">
    ...
    <div class="col-12">
        ...
        <input type="text" name="descripcion" id="descripcion" class="form-control" value="<?= htmlspecialchars($_POST['descripcion'] ?? '') ?>" required>
    </div>
    <div class="col-12">
