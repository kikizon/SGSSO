...
if (!in_array($tipo_filtro, ['acto_inseguro', 'accidente', 'enfermedad_cronica'])) $tipo_filtro = 'acto_inseguro';
...
if ($tipo_filtro == 'enfermedad_cronica') {
    ...
    if ($tipo_filtro == 'accidente') {
        ...
        if ($ultimo) { $hoy = new DateTime(); $fechaUltimo = new DateTime($ultimo); $dsa = $hoy->diff($fechaUltimo)->days; } else { $dsa = 'Sin datos'; }
    }
    $tasaFrecuencia = 0;
...
if ($tipo_filtro == 'enfermedad_cronica') {
    ...
    for ($m=1; $m<=12; $m++) { $mesesAnio[] = date('M', mktime(0,0,0,$m,1)); $actualAnio[] = 0; $anteriorAnio[] = 0; }
    ...
    if ($totalPareto > 0) {
        $acumulado = 0;
        foreach ($paretoData as $val) { $acumulado += $val; $paretoAcumulado[] = round(($acumulado / $totalPareto) * 100, 1); }
    } else { $paretoAcumulado = array_fill(0, count($paretoData), 0); }
    ...
    if ($totalCatalogo == 0) { $catalogoLabels = ['Sin datos']; $catalogoValores = [0]; }
    else {
    ...
    $horasLabels = []; for ($h=0; $h<24; $h++) $horasLabels[] = sprintf('%02d:00', $h);
    ...
    foreach ($res as $row) { $dia = $row['dia'] - 1; $hora = (int)$row['h']; $heatmapData[$dia][$hora] = (int)$row['total']; }
    $heatmapLabels = []; $heatmapValues = [];
    for ($d=0; $d<7; $d++) { for ($h=0; $h<24; $h++) { if ($heatmapData[$d][$h] > 0) { $heatmapLabels[] = $diasSemana[$d].' '.sprintf('%02d:00',$h); $heatmapValues[] = $heatmapData[$d][$h]; } } }
    ...
    $rangosEdad = ['< 25', '25-34', '35-44', '45-54', '55+']; $conteoEdad = [0,0,0,0,0];
    ...
    $res = $pdo->query($sqlEdad); foreach ($res as $row) $conteoEdad[$row['rango']] = (int)$row['total'];
    $diasSemanaFull = ['Lunes','Martes','Miércoles','Jueves','Viernes','Sábado','Domingo']; $reportesPorDia = array_fill(0,7,0);
    ...
    $res = $pdo->query($sqlDias); foreach ($res as $row) { $indice = ($row['dia']+5)%7; $reportesPorDia[$indice] = (int)$row['total']; }
    $compLabels = []; $compActos = []; $compAccidentes = [];
    ...
    foreach ($resComp as $row) { $compLabels[] = $row['item']; $compActos[] = (int)$row['actos']; $compAccidentes[] = (int)$row['accidentes']; }
    ...
    foreach ($paretoDataRaw as $row) { $paretoLabels[] = $row['descripcion']; $paretoData[] = (int)$row['total']; }
    $totalPareto = array_sum($paretoData);
    $paretoAcumulado = [];
    if ($totalPareto > 0) { $acumulado = 0; foreach ($paretoData as $val) { $acumulado += $val; $paretoAcumulado[] = round(($acumulado / $totalPareto) * 100, 1); } }
    else { $paretoAcumulado = array_fill(0, count($paretoData), 0); }
    if (empty($paretoLabels)) { $paretoLabels = ['Sin datos']; $paretoData = [0]; $paretoAcumulado = [0]; }
...
<div class="row mb-4">
    ...
    <?php else: ?>
        ...
        <div class="col-md-2"><div class="card bg-warning text-white h-100 kpi-card"><div class="card-body"><div class="d-flex justify-content-between align-items-center"><h6 class="card-title">Prom. Diario <i class="fas fa-info-circle" data-bs-toggle="tooltip" title="Promedio diario en el mes actual"></i></h6></div><h2 class="display-6"><?=$promedioDiario?></h2></div></div></div>
