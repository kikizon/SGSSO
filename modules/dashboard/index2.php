...
else
{
    ...
    // Total de días de incapacidad (acumulado)
    ...
    $relacionDiasVsCosto = 0;

    if ($tipo_filtro == 'accidente') {
        // AJUSTAR NOMBRES DE TABLAS/CAMPOS SEGÚN ESQUEMA REAL
        // Se asume tabla 'atenciones_particulares' con campos: costo, es_st7, dias_incapacidad, fecha, empleado_id, sucursal_id, departamento_id
        
        $filtroSucursalAtenciones = $sucursal_id ? "AND a.sucursal_id = $sucursal_id" : "";
        
        $costoTotalParticulares = $pdo->query("SELECT SUM(costo_atencion) FROM reportes r WHERE tipo = 'accidente' AND costo_atencion IS NOT NULL $filtroSucursalReportes")->fetchColumn();
    $costoTotalParticulares = $costoTotalParticulares ?: 0;
    
    // Total de atenciones que tienen costo registrado (particulares)
    $totalAtencionesParticulares = $pdo->query("SELECT COUNT(*) FROM reportes r WHERE tipo = 'accidente' AND costo_atencion IS NOT NULL $filtroSucursalReportes")->fetchColumn();
    
    // Atenciones clasificadas como ST7 (solo IMSS)
    $totalAtencionesST7 = $pdo->query("SELECT COUNT(*) FROM reportes r WHERE tipo = 'accidente' AND st7 = 1 $filtroSucursalReportes")->fetchColumn();
    
    // Total de atenciones IMSS (para calcular porcentaje ST7)
    $totalAtencionesIMSS = $pdo->query("SELECT COUNT(*) FROM reportes r WHERE tipo = 'accidente' AND atencion_medica_id IS NOT NULL AND (SELECT descripcion FROM atenciones_medicas WHERE id = r.atencion_medica_id) LIKE '%IMSS%' $filtroSucursalReportes")->fetchColumn();
    
    // Porcentaje de ST7 sobre IMSS
    $porcentajeST7 = ($totalAtencionesIMSS > 0) ? round(($totalAtencionesST7 / $totalAtencionesIMSS) * 100, 1) : 0;
    
    // Costo promedio por atención particular
    $costoPromedioParticular = ($totalAtencionesParticulares > 0) ? round($costoTotalParticulares / $totalAtencionesParticulares, 2) : 0;
    
    // Promedio de días de incapacidad por accidente (entre los que tienen incapacidad)
    $totalAccidentesConIncapacidad = $pdo->query("SELECT COUNT(*) FROM reportes r WHERE tipo = 'accidente' AND dias_perdidos > 0 $filtroSucursalReportes")->fetchColumn();
    $totalDiasIncapacidad = $pdo->query("SELECT SUM(dias_perdidos) FROM reportes r WHERE tipo = 'accidente' $filtroSucursalReportes")->fetchColumn();
    $promedioDiasIncapacidad = ($totalAccidentesConIncapacidad > 0) ? round($totalDiasIncapacidad / $totalAccidentesConIncapacidad, 1) : 0;
    
    // Tasa de incapacidad prolongada (>30 días)
    $accidentesProlongados = $pdo->query("SELECT COUNT(*) FROM reportes r WHERE tipo = 'accidente' AND dias_perdidos > 30 $filtroSucursalReportes")->fetchColumn();
    $totalAccidentes = $pdo->query("SELECT COUNT(*) FROM reportes r WHERE tipo = 'accidente' $filtroSucursalReportes")->fetchColumn();
    $tasaIncapacidadProlongada = ($totalAccidentes > 0) ? round(($accidentesProlongados / $totalAccidentes) * 100, 1) : 0;
    
    // Relación días perdidos vs costo (días por cada $1000 gastados)
    //$relacionDiasVsCosto = ($costoTotalParticulares > 0) ? round(($totalDiasIncapacidad / $costoTotalParticulares) * 1000, 2) : 0;
    }
}
...
<div class="row mb-4">
    ...
    <?php else: ?>
        ...
        <div class="col-md-2"><div class="card bg-warning text-white h-100 kpi-card"><div class="card-body"><div class="d-flex justify-content-between align-items-center"><h6 class="card-title">Total Días Incap. <i class="fas fa-info-circle" data-bs-toggle="tooltip" title="Suma total de días de incapacidad por accidentes"></i></h6></div><h2 class="display-6"><?=$totalDiasIncapacidad?></h2></div></div></div>
            ...
            <div class="card bg-success text-white h-100 kpi-card">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <h6 class="card-title">Costo Atenciones 
                            <i class="fas fa-info-circle" data-bs-toggle="tooltip" title="Costo total de atenciones particulares"></i>
                        </h6>
                    </div>
                    <h2 class="display-6">$<?=number_format($costoTotalAtenciones, 2)?>
                    </h2>
                </div>
            </div>
        </div>

        <div class="col-md-2"><div class="card bg-info text-white h-100 kpi-card"><div class="card-body"><div class="d-flex justify-content-between align-items-center"><h6 class="card-title">% ST7 <i class="fas fa-info-circle" data-bs-toggle="tooltip" title="Porcentaje de atenciones que fueron ST7"></i></h6></div><h2 class="display-6"><?=$porcentajeST7?>%</h2></div></div></div>
        <div class="col-md-2"><div class="card bg-warning text-white h-100 kpi-card"><div class="card-body"><div class="d-flex justify-content-between align-items-center"><h6 class="card-title">Prom. Días Incap. <i class="fas fa-info-circle" data-bs-toggle="tooltip" title="Promedio de días de incapacidad por accidente"></i></h6></div><h2 class="display-6"><?=$promedioDiasIncapacidad?></h2></div></div></div>
        ...
        <div class="col-md-2"><div class="card bg-danger text-white h-100 kpi-card"><div class="card-body"><div class="d-flex justify-content-between align-items-center"><h6 class="card-title">Incap. Prolongada <i class="fas fa-info-circle" data-bs-toggle="tooltip" title="% de accidentes con más de 30 días perdidos"></i></h6></div><h2 class="display-6"><?=$tasaIncapacidadProlongada?>%</h2></div></div></div>
        
        <!--<div class="col-md-2">
            <div class="card bg-dark text-white h-100 kpi-card">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <h6 class="card-title">Días/$1k Costo <i class="fas fa-info-circle" data-bs-toggle="tooltip" title="Días perdidos por cada $1000 de costo de atención"></i></h6>
                    </div>
                    <h2 class="display-6">$relacionDiasVsCosto?></h2>
                </div>
            </div>
        </div>-->
        <?php endif; ?>
